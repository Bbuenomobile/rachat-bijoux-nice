...
export async function submitContact(_prev: ContactState | undefined, formData: FormData): Promise<ContactState> {
  ...
  const from = process.env.RESEND_FROM || 'Rachat Bijoux Nice <contact@obagem.fr>';
    ...
    <div style="font-family:Inter,Arial,sans-serif;background:#faf8f3;padding:24px;">
      <div style="max-width:620px;margin:0 auto;background:#fff;border:1px solid #ecebe5;border-radius:14px;overflow:hidden;">
        <div style="background:#0c0b09;color:#f3e9c6;padding:18px 24px;">
          <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:22px;letter-spacing:.5px;">Rachat Bijoux Nice</div>
  ...
  try {
    const subject = `[Rachat Bijoux Nice] ${v.category} — ${v.name}`;
    ...
    try {
      await resend.emails.send({
        from,
        to: [v.email],
        subject: `Votre demande d'estimation — Rachat Bijoux Nice`,
        html: `
          <div style="font-family:Inter,Arial,sans-serif;background:#faf8f3;padding:24px;">
            <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #ecebe5;border-radius:14px;overflow:hidden;">
              <div style="background:#0c0b09;color:#f3e9c6;padding:18px 24px;font-family:'Cormorant Garamond',Georgia,serif;font-size:22px;">Rachat Bijoux Nice</div>
              <div style="padding:22px 24px;color:#2a2720;line-height:1.6;">
                ...
                <p style="color:#8c8675;font-size:12px;margin-top:24px;">Rachat Bijoux Nice — expertise gemmologique & rachat — 2 Avenue de Verdun, 06000 Nice</p>
              </div>
            </div>
          </div>`,
        text: `Bonjour ${v.name},\n\nNous avons bien reçu votre demande. Un expert vous contactera très rapidement.\n\nRachat Bijoux Nice — +33 1 48 87 81 44`,
      });
    } catch (e) {
